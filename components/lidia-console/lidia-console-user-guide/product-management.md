...
The Brands section is the area where the brands associated with products offered for sale on the marketplace, and any existing sub-brand structures, are centrally managed. Through this section, all brands in the system can be defined, edited, ordered, and consistently associated with category and product structures from a single location. Brand management plays a critical role in ensuring accurate product listings, a reliable filtering experience, the proper organization of category pages, and the healthy operation of campaign setups.

A new brand can be added to the system using the Add New Brand button located in the top-right corner of the Brands page. During the brand creation process, the brand name is entered, an optional parent brand is selected if applicable, and a page address (slug) for the brand is defined. This structure allows parent brand–sub-brand relationships to be configured hierarchically. Next to the Add New Brand button, the Export Data button enables all brands listed in the table to be exported in Excel format; this feature provides convenience for reporting, archiving, and data control processes.

Brand ordering is not an area that is available by default to all users within the Console. This capability is a feature that can be provided and configured by Lidia management when needed. When the brand ordering feature is activated, the priority order in which brands are listed on the marketplace can be managed, and brand showcase strategies can be shaped accordingly.
